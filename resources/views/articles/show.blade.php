<x-layouts::app :title="$article->title">
    <div class="w-full">
        <div class="max-w-3xl mx-auto px-4 py-6">
                        ...
                        @if(auth()->user()->isAdmin())
                            <div class="ml-auto flex items-center gap-2">
                                <a href="{{ route('articles.edit', $article->slug) }}"
                                   wire:navigate
                                   class="text-xs px-2 py-1 rounded text-zinc-500 hover:text-zinc-200 transition-colors">
                                    Edit
                                </a>
                                <form method="POST" action="{{ route('articles.destroy', $article->slug) }}"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="text-xs px-2 py-1 rounded text-zinc-500 hover:text-red-400 transition-colors">
                                        Delete
                                    </button>
                                </form>
                            </div>
            ...
            <div class="mt-8 pt-4 border-t border-zinc-700/40 flex items-center gap-4 flex-wrap">
    ...
    class="ml-auto inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-blue-500/10 border border-blue-500/30 text-blue-300 hover:bg-blue-500/15 transition-colors text-xs font-medium">
                    ...
                    <span><strong>{{ $commentCount }}</strong> {{ \Illuminate\Support\Str::plural('comment', $commentCount) }}</span>
                </button>
            </div>
